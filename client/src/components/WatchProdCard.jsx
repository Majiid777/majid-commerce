...
import { Link } from 'react-router-dom';
  ...
  export default function RecipeReviewCard({ montre }) {
  ...
  const { products } = useSelector(state => state.product);
    ...
    useEffect(() => {
      let result;
      result=parseFloat(calculeRemise(montre.price,montre.promo))
    ...
    return (
      ...
      <Card sx={{ width: "22rem" , height:"47rem" }}>
        ...
        <h4 className='promo2'>{`${montre.promo}% `}</h4>
        </div>:null}
        <CardMedia
          component="img"
          height="380 vh"
    ...
    { (Number(result)>0)?
       ...
       <CardContent className='Promotion'>
          <Typography >
          <p className='priceCardPromo' style={{  textDecoration: "line-through", marginTop:"8px",color: "black"}}>{`$ ${montre.price}`}</p>
          </Typography>
          <Typography  >
          <p className='priceCardPromo'>{`(${montre.promo}% off) `} </p>
          </Typography>
        </CardContent>
        <CardContent >
          <Typography   >
        ...
        <CardActions disableSpacing>
            ...
            <Button variant="dark" onClick={()=>dispatch(addToCart(montre._id, 1))} ><i style={{color:"white" , fontSize:'20px'}} className='fas fa-shopping-cart'></i></Button>
